...
Maze::Maze(const uint16_t height_, const uint16_t width_, const size_t seed_) :
        height(height_+(height_&1)), width(width_+(width_&1)), seed(seed_), grid(height*width){}

Maze::Maze(const uint16_t height_, const uint16_t width_): Maze(height_, width_, time(nullptr)){}
